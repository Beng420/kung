...
| Splits, personal bests, finish prediction, ideal time, TPS | `feature/dungeon/DungeonSplitTracker`, `DungeonSplitsOverlayFeature`; `config/category/SplitsConfig`; `util/ServerTickSequence`, `ServerTickPacketContext`, `ServerTpsTracker`, `KungDebugRecorder`; `mixin/ClientCommonPacketListenerMixin`, `ClientPacketListenerMixin` | `DungeonSplitTrackerTest`, `DungeonSplitPersonalBestsTest`, `config/SplitPersonalBestsConfigTest`, `DungeonSplitClockConservationTest`, `DungeonSplitPacketTimingTest`, `DungeonSplitsOverlayTest`, `ServerTpsTrackerTest`, `ServerTickPacketContextTest`, `KungDebugRecorderTest` | [Current split contract](RUN_STATISTICS.md#splits), [timing evidence](SPLITS_TIMING_FIXES.md) |
...
| Config / menu appearance / HUD editor | `config/KungConfig`, `KungConfigScreen`, `KungHudEditorScreen`; `config/category`, `ui/UiMenuFont`, `UiShapes`, `UiTheme`; `mixin/FontAccessor` | `ConfigRegressionTest`, `SettingEntryTest`, `UiControlModelTest`, `UiMenuFontTest` | [Development conventions](DEVELOPMENT.md#changing-code-and-comments) |
...
| Hypixel update popup / polling and cooldown / changelog history / update menu / GitHub | `update/KungUpdater`, `KungUpdateCheckSchedule`, `KungUpdateNotification`, `KungUpdateToast`, `KungUpdateToastState`, `KungReleaseNotes`, `KungReleaseHistory`, `KungReleaseNotesPopup`; `command/UserCommandGroup`, `KungCommandActions`; `config/KungConfigScreen` | `KungUpdateCheckScheduleTest`, `KungUpdateNotificationTest`, `KungUpdateToastStateTest`, `KungReleaseNotesTest`, `KungReleaseHistoryTest` | [Updates](UPDATES.md) |
