...
| Score, secrets, deaths, end summary | `feature/dungeon/DungeonRunStats`, `DungeonSecretCounts`, `DungeonSecretCounter`, `DungeonDeathTracker`, `DungeonRoomProgress`, `DungeonRunSummaryLayout` | `DungeonStatisticsSourcesTest`, `DungeonRunRosterRegressionTest`, `DungeonScoreCalculatorTest`, `DungeonSecretCounterTest`, `DungeonDeathTrackerTest`, `DungeonRunSummaryTest`, `DungeonRoomProgressTest`, `DungeonRunSummaryLayoutTest` | [Statistics](RUN_STATISTICS.md), [0.3.1 evidence](RUN_STATISTICS_FIXES.md) |
...
| Hypixel update notice / update menu / GitHub | `update/KungUpdater`, `KungUpdateNotification`; `command/UserCommandGroup`, `KungCommandActions`; `config/KungConfigScreen` | `KungUpdateNotificationTest` | [Updates](UPDATES.md) |
