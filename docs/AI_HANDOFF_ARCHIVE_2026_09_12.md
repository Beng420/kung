> Historical snapshot, archived on 2026-09-12 during documentation cleanup.
> Read [AI_HANDOFF.md](AI_HANDOFF.md) for current status and topic routing.
> This preserves earlier context, including superseded build counts and plans;
> it is not an additional set of current project instructions.
...
The September 12 follow-up, including the Lava Pit correction, split timing,
centered custom sounds and calculator profile selection, is documented in
[FOLLOWUP_FIXES_2026_09_12.md](FOLLOWUP_FIXES_2026_09_12.md). The proposed exact-position
filter for static Mimic chest candidates is in
[MIMIC_STATIC_CHESTS_PLAN.md](MIMIC_STATIC_CHESTS_PLAN.md). It is implemented in
**0.2.14** with explicit `/kung mimic ignore` capture, `/kung mimic` status and
`/kung mimic undo`; see [MIMIC_STATIC_CHESTS.md](MIMIC_STATIC_CHESTS.md).
No real chest coordinates are bundled yet. User-confirmed positions are stored
locally in `kung-dungeon-scans/static-trapped-chests.json`, bound to room cells,
hashes and independent structure probes. Never automatically learn an observed
Trapped Chest as static. Unknown rotations preserve candidates; normal scans
have a 512-block-read budget and cache resolved positions across chunk unloads.
Exact templates replace the Buttons/Slime whole-room fallback only for covered
variants. Full build: 259 tests passing; latest JAR is
`versions/mc26_1_2/build/libs/kung-26.1.2-0.2.14.jar`.

The floor-specific Stella boss-map assets, projection and switching rules are
documented in [BOSS_MAPS.md](BOSS_MAPS.md). The PNGs are bundled unchanged with Stella
and BetterMap attribution. **0.2.13** corrects only Necron's `f7_boss_s4`
metadata height from 94 to 114 blocks so the island center and arena edges align.
Full build: 244 tests passing; artifact `versions/mc26_1_2/build/libs/kung-26.1.2-0.2.13.jar`.
Boss rendering is separate from instance detection;
keep its selection and world-position markers separate from the clear-map grid.

The later split-clock regression is fixed in **0.2.12**: ignore zero ping IDs
without changing the last real tick ID, and bound ideal durations per phase.
Totals sum those bounded phases; never carry excess ticks into later splits or
erase a settled loss through catch-up in a different phase. See
[SPLITS_TIMING_FIXES.md](SPLITS_TIMING_FIXES.md). Full build: 243 tests passing;
the JAR is `versions/mc26_1_2/build/libs/kung-26.1.2-0.2.12.jar`.
...
For room names, secrets, shapes, crypts, and Prince metadata, the user explicitly chose the [Hypixel SkyBlock Wiki Catacombs Rooms page](https://hypixelskyblock.minecraft.wiki/w/Catacombs_Rooms) as the primary comparison source. Skyblocker remains a visual/implementation reference, not the room metadata authority. Use the saved [reference table](reference/catacombs-rooms.json) and its [source and interpretation notes](CATACOMBS_ROOMS_REFERENCE.md). The page is work in progress and contains duplicate `Lava Pit` rows; it has no `Lava Pool` entry or Kung hashes. Do not infer a hash-to-room identity from similar names.
...
- `HypixelSkyBlockProfileClient.loadCalculatorPlayer` is the common profile source for `/kung calculate cata`, `!c50` and `!ca50`: configured direct Hypixel access, otherwise/fallback Adjectils. Do not restore the obsolete PixelStats GUI endpoint. A selected profile takes priority over later profiles; only absent selection permits highest-dungeon-XP fallback. Adjectils supplies raw data and Kung calculates locally through `CatacombsAverageCalculator`. Catacombs Explorer is separate from the Expert Ring: GUI reads cumulative Epic shard thresholds into a level, permits manual adjustment and labels missing-data presets. Chat `!c50` uses the existing maximum-bonus projection, now including Explorer once.
...
- `ServerTpsTracker`: records ping receipt timestamps including bundled packets and deduplicates repeated IDs. `!tps` replies as `Current: <current> (max/min/avg) <max>/<min>/<avg>` in party, guild, all chat, or DMs according to its channel toggles. Dungeon split ticks instead come from `ClientCommonPacketListenerMixin.handlePing` at `TAIL`, in the same applied order as chat boundaries. Preserve that distinction; see [SPLITS_TIMING_FIXES.md](SPLITS_TIMING_FIXES.md).
...
`DungeonRoomRenderLayout` projects the final `roomOwners` and `internalDoors` into
shared shapes for map fills and labels. The raw catalog may conservatively return
several matches for one logical Layers room after an OPEN separator scan; rendering
those raw matches separately used to leave a gap and duplicate its label. Draw the
cached logical layout instead, including core hints. These render-only projections
must never be fed into room learning or persisted as catalog matches. A different
owner or a missing internal connection remains a boundary; groups stay bounded to
four cells. Build the layout once per render-plan identity, not once per frame.

`RoomType.RARE` is blue and follows ordinary room/door mechanics. Eight matched Rare
names in the saved wiki reference migrate from old NORMAL records without changing
other metadata. Remote legacy names and NORMAL-to-RARE updates are handled too.
The user-confirmed core `-1005518830` has only a bundled Rare type hint, not a
verified name/secret count. Preserve type-only blue rendering without inventing
catalog metadata; the render layout may refine an old NORMAL label to RARE while
leaving the raw learned match intact. Type learning invalidates the render plan.

Correction on 2026-09-12: the duplicated wiki name Lava Pit does not establish the
type of Kung's bundled Lava Pit. The user confirms that bundled room is normal.
It and the existing Lava Skull/Lava Tomb aliases are NORMAL, including repaired
older RARE imports and remote reports. Never reapply that name-only wiki migration.

The user confirmed the existing crypt totals on 2026-09-11: Admin = 34 and
Buttons = 21. Treat these project values as verified, even though the preserved
wiki snapshot still includes its historical `[Confirm]` markers. Do not restore
room-name-based uncertainty for either room or change their numbers from that marker.
...
- Player Tracking requests bounded room scans even with the map disabled. `DungeonRoomClearAttribution` retains first-clear witnesses and merges late-recognized room cells without duplicate credit. `DungeonRoomProgress` excludes Start/Fairy/Blood from displayed cleared/opened/total room counts and counts logical room owners, including white checkmarks; do not use completed-secret markers or tab cells as the displayed room total. Player room ranges remain, without the `estimated min-max` suffix. `DungeonRunSummaryLayout` measures names with the active font and uses Kung's one-pixel spacing font to align Secret columns. `DungeonBonusContribution` adds P/M/B only for explicit named claims or an observed Mimic damage-source player; global bonus events and generic party announcements do not identify a contributor.
- `DungeonDeathTracker` accepts anchored Hypixel system death messages for already-known players. It must never register an identity from a death line. Social/forwarded chat and actionbar text cannot change deaths. Exact `Team Deaths: n` tab/sidebar counters reconcile with per-player event counts without double-counting; UUID remaps merge aliases. The September 11 trace's forwarded Skyblocker line previously invented a player named `Party`.
- `DungeonSecretCounts` separates personal `Secrets Found: n` (tab, local player), party `Secrets Found: n%`, exact party `Secrets: n/total`, and room `n/total Secrets` actionbar data. Never associate nearby tab rows with the last party member, attribute a party delta to the local player/room, or count arbitrary found-secret chat. `DungeonSecretCounter` prioritizes personal tab over verified remote self-report over valid API delta; exact snapshots can correct a higher stale value. Unknown personal counts render `?`. Live sync publishes only a known personal self count and accepts only known roster members' own reports with timestamps; do not relay third-party guesses. Missing API achievements (-1) cannot establish a baseline. API callbacks from obsolete runs and baselines arriving after final fetch are ignored.
- Party Secrets uses a server count, compatible percentage plus a room-catalogue total, or a complete set of personal counts; an incomplete set is unknown. Map and summary share count logic. Catalogue totals are unavailable while an observed clearable owner lacks metadata, count each logical owner once, and are cached by render-plan identity. Once the server sends Score/Team Score, it takes priority over estimates; a final Team Score cannot be replaced by an ordinary later Score field.
...
- Built-in notification plings use listener-relative `SimpleSoundInstance.forUI` playback. Imported files use `CustomSoundPlayer` and `CustomSoundMixer`: one fixed-rate device stream, bounded overlapping voices, centered equal left/right samples, software pitch and preserved per-file volume. Disable/world reset invalidates pending decodes and closes the device stream. Sound packets are observed at applied `ClientPacketListener` TAIL, including bundles. Vanilla sound sources are unchanged. The Wither Shield item-use estimate must not restart on every repeated click.
...
- Footer secret display is `found - remaining for 300 - max`, not `found - target total - max`. Treat exact party `Secrets: found/available` as truth before deriving from a percentage and catalogue total. Reject a denominator if no integer secret count reproduces the displayed one-decimal percentage; unknown values show `?`. Trace category `score-calc` logs found/target/remaining/max, the actual score source, score estimates, failed puzzle count, crypts, and bonus flags whenever the calculation changes.
...
- Superpairs helper: `feature/misc/SuperpairsHelperFeature` adapts applied container packets to the pure `SuperpairsBoard` model. Glass button prompts are concealed markers, never bonuses; remember rewards by board slot through hiding and count duplicate reward pairs without capping a reward type at two cards. Only the 54-slot menu's interior board fields contribute. Keep observations on `ClientPacketListenerMixin` TAIL, not `ConnectionMixin`/Netty, and reset for a different menu object while retaining the same menu across screen reinitialization. Rendering consumes cached counts; do not restore per-frame inventory scans. Totals with unrevealed bonuses are upper bounds, and `Known` means matching identities observed, not necessarily claimed. Trace category `superpairs` records session, reveal and count changes independently of loadout diagnostics. See `docs/SUPERPAIRS_FIXES.md` for the September 11 trace and validation.
