...
Hideyho's own `You found Hideyho, and as a reward ...` capture is also supported.
...
The HUD is gated to Hypixel and exact `Critter Safari` / the four `... Biome`
location names. The Torrhus entrance does not qualify. During a new epoch with
...
Validation on 2026-09-14: all 16 focused Safari tests passed, followed by the full
Java 25 active-module build with 422 tests and no failures/errors/skips.
Artifact: `versions/mc26_1_2/build/libs/kung-26.1.2-0.3.2.jar`.
`git diff --check` also passed. No game-profile installation was performed.
...
Live checks remain: enable before entering, compare all four region counts with
own/shared catches, confirm reset in the next instance, verify the actual server
location spelling/entrance exclusion, and inspect readability/dragging/scaling
alongside the other HUDs. Unit tests and a build do not establish live results.
