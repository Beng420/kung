...
public final class SuperpairsHelperFeature {
    ...
    private static synchronized void renderAfterScreen(Screen screen, GuiGraphicsExtractor graphics) {
        ...
        if (snapshot.boardSlots() > 0) {
            boardSlotCount = Math.max(boardSlotCount, Math.min(MAX_PLAYABLE_FIELDS, snapshot.boardSlots()));
        }

        int bonusFields = discoveredBonusSlots.isEmpty()
            ? 0
            : Math.max(ASSUMED_BONUS_FIELDS, discoveredBonusSlots.size());
        int totalPairs = Math.max(0, (boardSlotCount - bonusFields) / 2);
        if (totalPairs <= 0 && !config.superpairsHelperDebugEnabled()) {
            return;
        }

        int discoveredPairTypes = discoveredCounts().size();
        int unseenPairs = Math.max(0, totalPairs - discoveredPairTypes);
    ...
    private static boolean isSuperpairsGameOpen(Minecraft client) {
        ...
        return isSuperpairsGameScreen(client.gui.screen());
    }
    ...
    private static boolean isBoardMarker(ItemStack stack) {
        ...
        return itemPath.contains("stained_glass");
    }
    ...
    private static boolean isVisibleCard(ItemStack stack) {
        ...
        return !itemPath.contains("stained_glass")
            ...
            && !name.contains("next")
            && !name.contains("start")
            && !name.contains("superpairs")
            && !name.contains("chronomatron")
            && !name.contains("ultrasequencer")
            && !name.contains("experimentation")
            && !name.contains("remaining")
            && !name.contains("clicks")
            && !name.contains("timer")
            && !name.contains("time left")
            && !name.contains("click to")
            && !name.contains("click a second")
            && !name.contains("button")
            && !name.contains("play again")
    ...
    private static String cleanName(ItemStack stack) {
        return stack.getHoverName().getString().replaceAll("\\s+", " ").trim();
    }
    ...
    private static void drawPanel(
        ...
        lines.add(totalPairs > 0 ? "Unseen Pairs: " + unseenPairs : "Superpairs Debug");
